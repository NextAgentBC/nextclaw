...
import { buildChatClient } from "../embedding/chat-client.js";
/* --------------------------------- helpers -------------------------------- */
const META_KEY = (cid, sessionId) => `shadow_comparator.${cid}.${sessionId}.offset`;
async function readOffset(pool, cid, sessionId) {
    const rows = await pool.query("SELECT value FROM audit.plugin_meta WHERE key = $1", [META_KEY(cid, sessionId)]);
    const v = rows.rows[0]?.value?.offset;
    return typeof v === "number" && Number.isFinite(v) && v >= 0 ? v : 0;
}
async function writeOffset(pool, cid, sessionId, offset) {
    await pool.query(`INSERT INTO audit.plugin_meta (key, value, updated_at)
       VALUES ($1, $2::jsonb, now())
     ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = now()`, [META_KEY(cid, sessionId), JSON.stringify({ offset })]);
}
...
/* ------------------------- trajectory scanning --------------------------- */
async function readTurnsSinceOffset(filePath, startOffset) {
    let info;
    try {
    ...
    if (info.size <= startOffset) {
        return { turns: [], finalOffset: info.size };
    }
    ...
    try {
        ...
        for (const [runId, evs] of byRun) {
            if (!evs["session.ended"] || !evs["model.completed"] || !evs["prompt.submitted"] || !evs["session.started"]) {
                continue;
            }
            ...
            const subData = (submitted.data ?? {});
            const compData = (completed.data ?? {});
            const usage = (compData["usage"] ?? {});
            const messages = coerceMessages(subData["messages"]);
            const userPrompt = typeof subData["prompt"] === "string" ? subData["prompt"] : undefined;
            const systemPrompt = typeof subData["systemPrompt"] === "string"
                ? subData["systemPrompt"]
                : undefined;
            ...
            turns.push({
                ...
                baseLatencyMs,
                baseInputTokens: typeof usage["input"] === "number" ? usage["input"] : undefined,
                baseOutputTokens: typeof usage["output"] === "number" ? usage["output"] : undefined,
                baseCacheRead: typeof usage["cacheRead"] === "number" ? usage["cacheRead"] : undefined,
                baseToolCount,
                baseOutput,
...
/* --------------------------------- ingest -------------------------------- */
async function shadowExists(pool, runId, chModel) {
    const r = await pool.query("SELECT 1 FROM audit.model_comparisons WHERE trajectory_run_id = $1 AND ch_model = $2", [runId, chModel]);
    return (r.rowCount ?? 0) > 0;
}
async function insertComparison(pool, row) {
    ...
    const outLenRatio = row.baseOutput && row.chOutput && row.chOutput.length > 0
        ...
        : null;
    await pool.query(`INSERT INTO audit.model_comparisons (
...
/* --------------------------------- entry ---------------------------------- */
export async function runShadowComparatorTick(deps) {
    const out = {
    ...
    try {
        ...
        out.errors.push(`readdir failed: ${err.message}`);
        return out;
    }
    ...
    for (const f of files) {
        ...
        try {
            ...
            out.errors.push(`read ${sessionId}: ${err.message}`);
            continue;
        }
        out.turnsSeen += scan.turns.length;
        for (const turn of scan.turns) {
            // Backfill cutoff: ignore stale turns at startup.
            if (turn.startedAt && Date.parse(turn.startedAt) < cutoff) {
                continue;
            }
            // Min length gate (don't waste GPU on "ok"-style turns).
            if ((turn.userPrompt?.length ?? 0) < deps.comparator.minUserMessageChars) {
                continue;
            }
            ...
            try {
                if (await shadowExists(deps.pool, turn.runId, client.model)) {
                    continue;
                }
            }
            catch (err) {
                out.errors.push(`exists check ${turn.runId}: ${err.message}`);
                continue;
            }
            // Run the challenger.
            const chReq = {
            ...
            try {
                ...
                out.errors.push(`insert ${turn.runId}: ${err.message}`);
            }
        }
        if (scan.finalOffset !== offset) {
            try {
                ...
                out.errors.push(`offset write ${sessionId}: ${err.message}`);
            }
        }
    }
    return out;
}
export function startShadowComparator(deps) {
    let timer = null;
    let stopped = false;
    const tick = async () => {
        if (stopped) {
        ...
        try {
            ...
            if (r.shadowsRun > 0 || r.errors.length > 0) {
                deps.logger?.info(`[shadow:${deps.comparator.id}] turns=${r.turnsSeen} shadowed=${r.shadowsRun} ` +
                    `failed=${r.shadowsFailed} errors=${r.errors.length}`);
            }
        }
        catch (err) {
            deps.logger?.warn(`[shadow:${deps.comparator.id}] tick failed: ${err.message}`);
        }
        if (!stopped) {
            timer = setTimeout(tick, deps.comparator.intervalMs);
        }
    };

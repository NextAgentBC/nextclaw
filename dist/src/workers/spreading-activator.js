 ...
 */
export async function spreadActivation(pool, hitChunkIds, bump = 0.1, neighbourCap = 20) {
    if (hitChunkIds.length === 0) {
        return 0;
    }
    // Find chunks sharing ≥ 2 (kind, value) pairs with a hit chunk; bump warmth.
    const result = await pool.query(`WITH neighbours AS (
     ...
     UPDATE semantic.chunks
      ...
      RETURNING id::text AS updated`, [hitChunkIds, bump, neighbourCap]);
 ...
 */
export async function bumpIndexWeights(pool, hitChunkIds, delta = 0.1) {
    if (hitChunkIds.length === 0) {
        return 0;
    }
    const result = await pool.query(`UPDATE semantic.chunk_indexes
        SET weight = weight + $2,
            hit_count = hit_count + 1
      WHERE chunk_id = ANY($1::uuid[])`, [hitChunkIds, delta]);
    return result.rowCount ?? 0;
}

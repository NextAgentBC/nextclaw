 ...
 * (it'll be a fragment).
 */
async function readJsonlTail(path) {
        ...
        if (obj.type === "message" && obj.role === "assistant") {
            ...
            if (typeof c === "string" && c.length >= MIN_SUMMARY_CHARS) {
                ...
                const joined = c
                    ...
                    .join("\n")
                    .trim();
                if (!lastAssistant && joined.length >= MIN_SUMMARY_CHARS) {
...
export async function ingestCompactionSummary(deps, ctx) {
    try {
        ...
        const outcome = await ingestOne(deps, {
            text: summary,
        ...
        deps.logger?.info?.(`compaction-ingest: agent=${ctx.agentId} session=${ctx.agentSessionId ?? "?"} ` +

...
import { computeRecallScore } from "../scoring.js";
import { bumpIndexWeights, spreadActivation, } from "../workers/spreading-activator.js";
import { mergeRoutes, mmrRerank, routeAnchor, routeCategory, routeConceptTag, routeEntityRef, routeFullText, routeGraphWalk, routeSemantic, routeTimeBucket, routeTrgm, } from "./routes.js";
import { categorize } from "../structured/categorizer.js";
import { inferTimeBucketsFromQuery } from "./temporal.js";
import { filterVisibleChunkIds } from "./viewer-scope.js";
 ...
 */
function inferAnchorsFromQuery(query, given) {
    const out = { ...(given ?? {}) };
    if (!out.pr) {
...
function inferConceptTagsFromQuery(query) {
    const seen = new Set();
    const add = (t) => {
...
/* --------------------------------- helpers --------------------------------- */
function hashQuery(query, scope) {
    return {
...
/* ---------------------------- cache.recall I/O ---------------------------- */
async function readRecallCache(pool, qHash, scopeKey) {
    const rows = await pool.query(`SELECT top_k FROM cache.recall
       WHERE query_hash = $1 AND scope_key = $2
         AND invalidated = false AND expires_at > now()
       LIMIT 1`, [qHash, scopeKey]);
    if (rows.rowCount === 0) {
        return null;
    }
...
async function writeRecallCache(pool, qHash, scopeKey, ttlSec, topK, routes) {
    await pool.query(`INSERT INTO cache.recall (query_hash, scope_key, top_k, expires_at)
       VALUES ($1, $2, $3::jsonb, now() + ($4 || ' seconds')::interval)
       ON CONFLICT (query_hash, scope_key) DO UPDATE
         SET top_k = EXCLUDED.top_k,
             expires_at = EXCLUDED.expires_at,
             invalidated = false,
             created_at = now()`, [qHash, scopeKey, JSON.stringify({ results: topK, routes }), ttlSec]);
}
/* ------------------------------- T1 promote ------------------------------- */
async function promoteToHotCache(pool, cfg, ids) {
    if (ids.length === 0) {
        return;
    }
    const ttlDays = cfg.tiers.t1TtlDays;
    // Bump warmth_score on the chunk and upsert into cache.hot_chunks.
    await pool.query(`UPDATE semantic.chunks
    ...
    await pool.query(`INSERT INTO cache.hot_chunks (user_scope, chunk_id, expires_at, warmth_score)
       ...
       ON CONFLICT (user_scope, chunk_id) DO UPDATE
         SET expires_at   = EXCLUDED.expires_at,
             ...
             promoted_at  = now()`, [ids, String(ttlDays)]);
}
/* ---------------------------------- recall --------------------------------- */
export async function recall(deps, input) {
    ...
    const ws = workingSetFor(input.agentSessionId, deps.cfg.tiers.t0SizeLimit, input.agentId, input.viewer?.userId);
    ...
    if (!ws.hasProfilesPrimed()) {
        await primeWorkingSetWithProfiles(deps.pool, input.agentId ?? "main", ws, deps.cfg.tiers.t0SizeLimit).catch(() => 0);
    }
    // ---------- T0 working set (in-process; sub-ms) ----------
    const t0Hits = ws.search(input.query, k);
    if (t0Hits.length >= Math.max(1, Math.ceil(k / 2))) {
        const tier = "t0";
        const score = computeRecallScore({ hitTier: tier, llmTokensUsed: 0, latencyMs: Date.now() - start, relevanceEstimate: 0.7 }, deps.cfg.scoring.recall);
        for (const c of t0Hits) {
            ws.touch(c.chunkId);
        }
    ...
    if (cached) {
        ...
        const tier = "t1";
        const score = computeRecallScore({ hitTier: tier, llmTokensUsed: 0, latencyMs: Date.now() - start, relevanceEstimate: 0.6 }, deps.cfg.scoring.recall);
        await auditRecall(deps, input, {
            hitTier: tier,
    ...
    const inferredTags = input.conceptTags && input.conceptTags.length > 0
        ? input.conceptTags
    ...
    const [anchorRows, entityRefRows, timeBucketRows, conceptTagRows, categoryRows, graphWalkRows,] = await Promise.all([
    ...
    const shouldRunSemantic = !hasPreciseAnchor || structuredFlat.length < Math.ceil(k / 2);
    ...
    if (shouldRunSemantic) {
        ...
        const ctxFull = { ...ctxBase, queryEmbedding };
    ...
    if (input.viewer && (input.viewer.userId || input.viewer.chatId) && filtered.length > 0) {
        const visible = await filterVisibleChunkIds(deps.pool, input.viewer, filtered.map((c) => c.chunkId));
...
/* ------------------------------- audit write ------------------------------- */
async function auditRecall(deps, input, partial) {
    const id = randomUUID();
    await deps.pool

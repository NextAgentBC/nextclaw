...
import { newModeratorState } from "./types.js";
export async function loadModeratorState(pool, agentId, scopeKey) {
    const r = await pool.query(`SELECT state, status FROM moderator.state WHERE agent_id = $1 AND scope_key = $2`, [agentId, scopeKey]);
    if (r.rowCount === 0) {
        return newModeratorState(scopeKey, scopeKey.startsWith("tg:chat:") ? "group" : "dm");
    }
    return migrateState(r.rows[0].state);
}
export async function saveModeratorState(pool, agentId, scopeKey, state, opts = {}) {
    ...
    await pool.query(`INSERT INTO moderator.state (
 ...
 */
export async function logDecision(pool, params) {
    const id = randomUUID();
    await pool.query(`INSERT INTO moderator.decisions (

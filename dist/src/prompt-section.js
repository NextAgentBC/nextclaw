 ...
 */
export const makeBuildPromptSection = (opts) => ({ availableTools, citationsMode }) => {
    ...
    if (!hasSearch && !hasStore && !hasUpdate && !hasForget) {
        return [];
    }
    const lines = ["## Memory Recall (memory-postgres)"];
    if (hasSearch) {
        lines.push("Before answering questions about past work, decisions, people, preferences, files, "
    ...
    if (hasStore) {
        lines.push("When the user states a durable fact, preference, decision, or shares a measurable "
    ...
    if (hasUpdate || hasForget) {
        ...
        lines.push("Memory is not append-only. You are expected to **curate** it as facts change:");
        if (hasUpdate) {
            lines.push("- `memory_update(chunkId, ...)` — rewrite a chunk's text, change its importance, or "
        ...
        if (hasForget) {
            lines.push("- `memory_forget(chunkId, ...)` — soft-trash a chunk so it stops appearing in recall. "
        ...
        lines.push("Both operations take a `chunkId` from a recent `memory_search` result. Cross-agent edits "
    ...
    if (opts.emitSidecar) {
        lines.push("## Memory Sidecar", "After your reply, on the very last line of your turn output exactly one structured "
            + "block in this format (omit if nothing memorable happened):", "<mem>{\"entities\":[],\"events\":[],\"preferences\":[],\"metrics\":[]}</mem>", "Each item is small and concrete (e.g. {\"type\":\"person\",\"canonicalName\":\"shadow\"}, "
            + "{\"metric\":\"calories\",\"value\":1800,\"unit\":\"kcal\",\"ts\":\"2026-05-02\"}).", "Sidecar is the cheapest path; deterministic regex + tool calls back you up if you skip it.");
    }
    if (citationsMode === "off") {
        lines.push("Citations are disabled: do not include source paths or line numbers in replies unless the user explicitly asks.");
    }
    else {
        lines.push("Citations: include `pg://<source>/<chunkId>` when it helps the user verify a memory.");
    }
    lines.push("");
    return lines;
};

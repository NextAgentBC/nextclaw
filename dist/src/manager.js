...
export class PostgresMemoryManager {
    ...
    status() {
        const embeddingProbe = this.cachedProbe ?? { ok: false };
        return {
    ...
    async probeVectorAvailability() {
        try {
            const result = await this.deps.pool.query("SELECT (count(*) > 0) AS ok FROM pg_extension WHERE extname = 'vector'");
    ...
    async maybeRecordDims(dims) {
        ...
        try {
            ...
            console.warn("[memory-postgres] failed to record embedding.dims:", err.message);
        }
    }
}
 ...
 */
export async function writeChunk(pool, embedding, input) {
    const id = randomUUID();
    ...
    try {
        await pool.query(`INSERT INTO semantic.chunks
        ...
        await pool.query(`INSERT INTO semantic.chunk_indexes (chunk_id, kind, value)
         VALUES ($1, 'time_bucket', $2)
         ON CONFLICT DO NOTHING`, [id, today]);
        if (input.agentSessionId) {
            await pool.query(`INSERT INTO semantic.chunk_indexes (chunk_id, kind, value)
           VALUES ($1, 'anchor_session', $2)
           ON CONFLICT DO NOTHING`, [id, input.agentSessionId]);
        }

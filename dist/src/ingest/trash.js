...
export function trashFilter(text) {
    const trimmed = text.trim();
    if (trimmed.length === 0) {
        return { ok: false, reason: "too_short", matched: "empty" };
    }
    // Boilerplate first — even short content matches here.
    for (const re of BOILERPLATE) {
        if (re.test(trimmed)) {
            return { ok: false, reason: "boilerplate", matched: re.source };
        }
    }
    // Pure stack trace or grep dump.
    if (STACKTRACE.test(trimmed)) {
        return { ok: false, reason: "tool_output_only", matched: "stacktrace" };
    }

...
import { startModeratorService } from "./src/moderator/service.js";
/**
 ...
 */
let MODULE_MODERATOR_HANDLE = null;
/** Module-level handles for the event-driven hooks (after_compaction,
 *  session_end). Same closure-isolation rationale as above. */
let MODULE_TRANSCRIPT_WATCHERS = [];
let MODULE_REFLECTION_HANDLE = null;
let MODULE_INGEST_DEPS = null;
let MODULE_DASHBOARD_CMD = null;
/**
 ...
 */
async function handleDashboardCommand(api, ctxObj) {
    const cmd = MODULE_DASHBOARD_CMD;
    if (!cmd) {
        api.logger.warn("dashboard cmd: not configured (publicUrl + telegram bot token both required)");
        return false;
    }
    const senderRaw = (ctxObj.senderId ?? "").replace(/^(?:channel:|chat:|user:|tg-?|telegram:|slack:|discord:|whatsapp:)/, "");
    if (!cmd.ownerIds.has(senderRaw)) {
        ...
        const chatId = (ctxObj.conversationId ?? "").replace(/^(?:channel:|chat:|user:|tg-?|telegram:|slack:|discord:|whatsapp:)/, "");
        if (chatId) {
...
export default definePluginEntry({
    ...
    description: "Postgres + pgvector memory plugin with 4-tier recall, multi-key indexing, scoring and self-tuning",
    kind: "memory",
    register(api) {
        ...
        const emitSidecar = api.pluginConfig
            ?.sidecar?.enabled === true;
        api.logger.info(`memory-postgres: sidecar prompt ${emitSidecar ? "enabled" : "disabled (default; set sidecar.enabled:true to opt in)"}`);
        ...
        api.on("before_dispatch", async (event, hookCtx) => {
            const ctxObj = hookCtx;
            if (ctxObj?.channelId !== "telegram") {
                return;
            }
            const ev = event;
            ...
            if (!isGroup) {
                return;
            } // DMs continue to codex
            const addressed = /@\w+_bot|@bot\b|^\/(start|ask|help)/i.test(content);
            if (!addressed) {
                return;
            } // group chatter not for the bot — codex won't reply anyway
            api.logger.info(`[moderator/hook] before_dispatch CLAIMED: conv=${ctxObj.conversationId} ` +
        ...
        api.on("message_received", async (event, hookCtx) => {
            ...
            if (!MODULE_MODERATOR_HANDLE) {
                return;
            }
            try {
                MODULE_MODERATOR_HANDLE.onMessageReceived(event, hookCtx);
            }
            catch (err) {
                api.logger.warn(`memory-postgres: moderator message_received hook threw: ${err.message}`);
            }
        });
        /**
        ...
        api.on("after_compaction", async (event, hookCtx) => {
            const ev = event;
            if (!ev?.sessionFile || !ev.compactedCount) {
                return;
            }
            const agentId = hookCtx?.agentId ?? "main";
            const agentSessionId = hookCtx?.sessionId
                ?? hookCtx?.sessionKey;
            if (!MODULE_INGEST_DEPS) {
                return;
            }
            void ingestCompactionSummary({ ...MODULE_INGEST_DEPS, logger: { info: (m) => api.logger.info(m), warn: (m) => api.logger.warn(m) } }, {
        ...
        api.on("session_end", async (event, hookCtx) => {
            const ev = event;
            const ctxAgent = hookCtx?.agentId ?? "main";
            ...
            for (const w of MODULE_TRANSCRIPT_WATCHERS) {
                w.flushNow().catch((err) => api.logger.warn(`session_end transcript flush failed (${w.watcherId}): ${err.message}`));
            }
            ...
            if (MODULE_REFLECTION_HANDLE && productiveReasons.has(reason)) {
                void MODULE_REFLECTION_HANDLE.triggerForAgent(ctxAgent, `session_end:${reason}`)
                    .catch((err) => api.logger.warn(`session_end reflection trigger failed: ${err.message}`));
            }
        });
        /**
        ...
        let compactorTimer = null;
        let gitWatcherHandles = [];
        let transcriptWatcherHandles = [];
        let shadowHandles = [];
        let reflectionHandle = null;
        let moderatorHandle = null;
        let moderatorEventUnsub = null;
        let activePoolCfg = null;
        api.registerService({
            id: SERVICE_ID,
            async start(ctx) {
                const rawCfg = ctx.config;
                const raw = rawCfg.plugins?.entries?.["memory-postgres"]?.config;
                if (!raw) {
                    api.logger.warn("memory-postgres: service start aborted — plugin config missing");
                    return;
                }
                try {
                    validateConfig(raw);
                    ...
                    api.logger.warn(`memory-postgres: service start aborted — config invalid: ${err.message}`);
                    return;
                }
                ...
                try {
                    ...
                    await ensureHnswIndex(pool).catch((err) => api.logger.warn(`memory-postgres: HNSW index not built (T2 hybrid will fall back to seq scan): ${err.message}`));
                }
                catch (err) {
                    api.logger.warn(`memory-postgres: schema migrate failed (will retry on first op): ${err.message}`);
                }
                // Dashboard.
                if (cfg.dashboard.enabled) {
                    try {
                        ...
                        const liveCfg = ctx.config;
                        const tgBotToken = liveCfg.channels?.telegram?.botToken;
                        const allowedTgIds = new Set((liveCfg.commands?.ownerAllowFrom ?? [])
                            .filter((s) => typeof s === "string" && s.startsWith("telegram:"))
                        ...
                        if (cfg.dashboard.publicUrl && tgBotToken && allowedTgIds.size > 0) {
                            ...
                            api.logger.info(`memory-postgres: /dashboard command armed → publicUrl=${cfg.dashboard.publicUrl}`);
                        }
                        else if (cfg.dashboard.publicUrl) {
                            api.logger.warn("memory-postgres: dashboard.publicUrl set but missing telegram bot token or ownerAllowFrom — /dashboard command not armed");
                        }
                    }
                    catch (err) {
                        api.logger.warn(`memory-postgres: dashboard failed to start: ${err.message}`);
                    }
                }
                else {
                ...
                const tuningTick = async () => {
                    try {
                        ...
                        if (analyzer.proposalIds.length > 0 || guards.evaluated > 0) {
                            api.logger.info(`memory-postgres: tuning tick — proposals=${analyzer.proposalIds.length} `
                                + `guards_evaluated=${guards.evaluated} guards_reverted=${guards.reverted}`);
                        }
                    }
                    catch (err) {
                        api.logger.warn(`memory-postgres: tuning tick failed: ${err.message}`);
                    }
                };
                ...
                const compactorTick = async () => {
                    try {
                        ...
                        if (outcome.gistsWritten > 0 || outcome.chunksDemoted > 0) {
                            api.logger.info(`memory-postgres: compactor — clusters=${outcome.clustersFound} `
                                + `gists=${outcome.gistsWritten} demoted=${outcome.chunksDemoted}`);
                        }
                    }
                    catch (err) {
                        api.logger.warn(`memory-postgres: compactor tick failed: ${err.message}`);
                    }
                };
                ...
                if (cfg.gitWatchers.length > 0) {
                    ...
                    for (const watcher of cfg.gitWatchers) {
                        ...
                        api.logger.info(`memory-postgres: git-watcher started — id=${watcher.id} `
                ...
                if (cfg.transcriptWatchers.length > 0) {
                    ...
                    for (const watcher of cfg.transcriptWatchers) {
                        ...
                        api.logger.info(`memory-postgres: transcript-watcher started — id=${watcher.id} `
                ...
                if (cfg.reflection.enabled && cfg.reflection.model.baseUrl) {
                    try {
                        ...
                        api.logger.info(`memory-postgres: reflection daemon started — ` +
                            `format=${cfg.reflection.model.format} model=${cfg.reflection.model.model} ` +
                            `intervalMs=${cfg.reflection.intervalMs}`);
                    }
                    catch (err) {
                        api.logger.warn(`memory-postgres: reflection daemon failed to start: ${err.message}`);
                    }
                }
                else if (cfg.reflection.enabled) {
                    api.logger.warn("memory-postgres: reflection.enabled=true but model.baseUrl is empty — daemon NOT started.");
                }
                ...
                if (cfg.moderator.enabled && cfg.moderator.model.baseUrl) {
                    try {
                        ...
                        if (!botToken) {
                            api.logger.warn("memory-postgres: moderator.enabled=true but channels.telegram.botToken empty — service NOT started.");
                        }
                        else if (!ingestToken) {
                            api.logger.warn("memory-postgres: moderator.enabled=true but dashboard.tokenEnv unresolved — service NOT started (memory-writes would fail).");
                        }
                        else {
                            ...
                            api.logger.info(`memory-postgres: moderator service started — model=${cfg.moderator.model.format}:${cfg.moderator.model.model} ` +
                                `debounceMs=${cfg.moderator.debounceMs} owner=${ownerUserId ?? "(none)"}`);
                        }
                    }
                    catch (err) {
                        api.logger.warn(`memory-postgres: moderator service failed to start: ${err.message}`);
                    }
                }
                else if (cfg.moderator.enabled) {
                    api.logger.warn("memory-postgres: moderator.enabled=true but model.baseUrl is empty — service NOT started.");
                }
                ...
                if (cfg.shadowComparators.length > 0) {
                    ...
                    for (const comparator of cfg.shadowComparators) {
                        ...
                        api.logger.info(`memory-postgres: shadow-comparator started — id=${comparator.id} ` +

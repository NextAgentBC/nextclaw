...
export async function reconcile(pool: Pool, input: ReconcileInput): Promise<ReconcileOutcome> {
...
async function upsertEntity(
  ...
  const existing = await client.query<{ id: string; aliases: string[] }>(
    `SELECT id, aliases FROM structured.entities
       WHERE type = $1 AND canonical_name = $2 AND deleted_at IS NULL
       LIMIT 1`,
    [e.type, e.canonicalName],
  );
  if (existing.rowCount && existing.rowCount > 0) {
    const row = existing.rows[0];
    if (!row) {return { id: existing.rows[0].id, deduped: true };}
  ...
  const id = randomUUID();
  await client.query(
    `INSERT INTO structured.entities
...
function mergeAliases(prev: string[], next: string[]): string[] {
...
async function upsertRelation(
  ...
  const id = randomUUID();
  await client.query(
    `INSERT INTO structured.relations
...
async function upsertPreference(
  ...
  if (existing.rowCount && existing.rowCount > 0) {
    ...
    const id = randomUUID();
    await client.query(
      `INSERT INTO structured.preferences
         (id, scope, key, value, rule_text, evidence_count, confidence, supersedes)
       VALUES ($1, $2, $3, $4, $5, 1, $6, $7)`,
      [id, p.scope, p.key, JSON.stringify(p.value), p.ruleText ?? null, p.confidence, row.id],
    );
    return { id, deduped: false };
  }
  const id = randomUUID();
  await client.query(
    ...
    [id, p.scope, p.key, JSON.stringify(p.value), p.ruleText ?? null, p.confidence],
  );
  return { id, deduped: false };
}
...
async function upsertMetric(
  ...
  const existing = await client.query<{ id: string }>(
    `SELECT id FROM structured.metrics
  ...
  const id = randomUUID();
  await client.query(
    `INSERT INTO structured.metrics
       (id, ts, metric, value, unit, dim, confidence)
     VALUES ($1, $2, $3, $4, $5, $6, $7)`,
    [id, m.ts, m.metric, m.value, m.unit ?? null, m.dim ?? {}, m.confidence],
  );
  return { id, deduped: false };
}
...
async function writeProvenance(
  ...
  for (const id of out.entityIds)     {rows.push(["entity", id, "entities"]);}
  for (const id of out.relationIds)   {rows.push(["relation", id, "relations"]);}
  ...
  for (const [kind, itemId] of rows) {
    await client.query(
      `INSERT INTO structured.provenance

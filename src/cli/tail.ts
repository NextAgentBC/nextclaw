...
export type TailFilter = "all" | "rejected" | "accepted" | "recall" | "ingest";
...
function formatLine(e: Event): string {
...
function shouldShow(e: Event, filter: TailFilter): boolean {
  switch (filter) {
...
export async function startTail(
  ...
  client.on("notification", (msg) => {
    ...
    try {
      const event = JSON.parse(msg.payload) as Event;

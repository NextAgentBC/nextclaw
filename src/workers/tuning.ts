...
export type RiskClass = "safe_auto" | "review_required" | "high_risk";
...
export type TuningProposal = {
  ...
  cadence: Cadence;
  scope: string;
  proposalType: "add" | "remove" | "adjust" | "replace";
...
async function writeProposal(
  pool: Pool,
  cadence: Cadence,
  payload: Omit<TuningProposal, "id" | "ts" | "status" | "cadence">,
): Promise<string> {
  const id = randomUUID();
 ...
 */
export async function runDailyAnalyzer(pool: Pool): Promise<AnalyzerResult> {
  const proposalIds: string[] = [];
  const notes: string[] = [];

  // 1. Recall cache hit-rate.
  const cacheStat = await pool.query<{
    total: string;
    cached: string;
  }>(
  ...
  if (total >= 30) {
    ...
    if (rate < 0.30) {
      proposalIds.push(
        await writeProposal(pool, "daily", {
  ...
  const trashStat = await pool.query<{
    rejected: string;
    boilerplate: string;
  }>(
    `SELECT count(*)::text AS rejected,
  ...
  const scores = await pool.query<{ short_avg: number | null; long_avg: number | null }>(
    `SELECT
...
export async function markAutoApplied(
  ...
  await pool.query(
    `UPDATE audit.tuning_proposals
        SET status = 'auto_applied',
            applied_at = now(),
            rollback_value = $2::jsonb
      WHERE id = $1`,
    [proposalId, JSON.stringify(rollbackValue)],
  );
  await pool.query(
    `INSERT INTO audit.tuning_guards
...
export async function markRejected(pool: Pool, proposalId: string, reason: string): Promise<void> {
  await pool.query(
    `UPDATE audit.tuning_proposals
...
export async function markApproved(pool: Pool, proposalId: string): Promise<void> {
  await pool.query(
    `UPDATE audit.tuning_proposals
...
export async function revertProposal(
  ...
  const rows = await pool.query<{ rollback_value: unknown }>(
    `UPDATE audit.tuning_proposals
        SET status = 'reverted', reverted_at = now(), reason = $2
      WHERE id = $1
      RETURNING rollback_value`,
    [proposalId, reason],
  );
 ...
 */
export async function evaluateGuards(pool: Pool): Promise<{
  evaluated: number;
  reverted: number;
  notes: string[];
}> {
  const guards = await pool.query<{
    ...
    `SELECT proposal_id, baseline_score, applied_at
  ...
  for (const g of guards.rows) {
    const observed = await pool.query<{ avg_score: number | null }>(
      `SELECT avg(score) AS avg_score FROM audit.recall_decisions
         WHERE ts > $1 AND score IS NOT NULL`,
      [g.applied_at],
    );
    ...
    await pool.query(
      `UPDATE audit.tuning_guards
          SET observed_score = $2,
              observed_window = tstzrange($3, now()),
              decision = $4,
              decided_at = now()
        WHERE proposal_id = $1`,
      [g.proposal_id, obs, g.applied_at, decision],
    );

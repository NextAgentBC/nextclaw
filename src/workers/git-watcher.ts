...
import type { ResolvedGitWatcher, ResolvedMemoryPostgresConfig } from "../config.js";
...
export type GitWatcherOutcome = {
  ...
  errors: string[];
  /** False when nothing new since last_sha — saves all the parsing. */
  hadNewCommits: boolean;
};

const META_KEY = (watcherId: string): string => `git_watcher.${watcherId}.last_sha`;
...
async function readLastSha(pool: Pool, watcherId: string): Promise<string | null> {
  const rows = await pool.query<{ value: { sha?: string } }>(
...
async function writeLastSha(pool: Pool, watcherId: string, sha: string): Promise<void> {
  await pool.query(
    `INSERT INTO audit.plugin_meta (key, value, updated_at)
       VALUES ($1, jsonb_build_object('sha', $2::text), now())
       ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = now()`,
    [META_KEY(watcherId), sha],
  );
}

function makeRunner(watcher: ResolvedGitWatcher): (args: string[]) => Promise<string> {
  return async (args) => {
...
export function parseCommits(raw: string): GitCommit[] {
  ...
  for (const block of blocks) {
    ...
    const files = filesPart
      ...
      .map((f) => f.trim())
      .filter((f) => f.length > 0);
    if (sha) {out.push({ sha, isoDate, author, authorEmail, subject, body, files });}
  }
  return out;
}

function formatChunk(c: GitCommit): string {
...
function extractPrNumber(c: GitCommit): string | undefined {
...
export async function pollGitWatcher(deps: GitWatcherDeps): Promise<GitWatcherOutcome> {
  ...
  try {
    ...
    errors.push(`rev-parse failed: ${(err as Error).message}`);
    return {
      watcherId: watcher.id,
      fromSha,
      toSha: fromSha ?? "",
  ...
  for (const c of ordered) {
    try {
      ...
      const outcome: IngestOutcome = await ingestOne({ cfg, pool, embedding }, input);
      ...
      errors.push(`commit ${c.sha.slice(0, 8)}: ${(err as Error).message}`);
    }
  }
  ...
  await writeLastSha(pool, watcher.id, toSha);
  log.info(
    `git-watcher[${watcher.id}]: ingested ${accepted} new + ${merged} merged + ${rejected} rejected `
...
export type GitWatcherHandle = {
  watcherId: string;
  stop: () => void;
};

export function startGitWatcherDaemon(
  deps: Omit<GitWatcherDeps, "runGit"> & { runGit?: GitWatcherDeps["runGit"] },
): GitWatcherHandle {
  const log = deps.logger ?? { info: () => undefined, warn: () => undefined };
  let stopped = false;
  ...
  const tick = async (): Promise<void> => {
    ...
    try {
      ...
      log.warn(`git-watcher[${deps.watcher.id}] tick failed: ${(err as Error).message}`);
    }
  };
